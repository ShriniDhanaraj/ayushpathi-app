...
export default function ReceptionistQueueScreen() {
  ...
  async function load() {
    ...
    // Get receptionist's hospital_id
              ...
              <View style={s.cardRow}>
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    ...
                    {apt.is_walk_in && <View style={s.walkInBadge}><Text style={s.walkInText}>Walk-in</Text></View>}
                  </View>
                  <Text style={s.subText}>
                    {apt.start_time?.slice(0, 5)} · Dr. {apt.doctor?.first_name} {apt.doctor?.last_name}
                  </Text>
                  <Text style={s.mobileText}>{apt.patient?.mobile}</Text>
                </View>
                <View style={[s.statusBadge, { backgroundColor: STATUS_COLOR[apt.status] + '20' }]}>
                  <Text style={[s.statusText, { color: STATUS_COLOR[apt.status] }]}>{apt.status}</Text>
                </View>
              </View>
              ...
              {nextStatuses.length > 0 && (
                <View style={s.actionRow}>
                  ...
                  {apt.status === 'COMPLETED' && (
                    <TouchableOpacity
                      onPress={() => router.push({ pathname: '/(receptionist)/prescription', params: { appointmentId: apt.id, patientName: `${apt.patient?.first_name} ${apt.patient?.last_name}` } })}
